...
# Process-wide title-fragment classifier reused across the fragment filter's
# per-line title check. The fragment filter calls it only for lines whose
# embedding similarity to the page title is ambiguous, but a PDF can produce many
# such lines; constructing the classifier (which calls ``init_model``) once and
# caching it here keeps that off the per-line path.
_title_fragment_classifier: Optional[object] = None


def get_title_fragment_classifier() -> object:
    """Return the shared title-fragment classifier, building it once on first use.

    The ``TitleFragmentClassificationClass`` import is local to keep the model/SDK
    off the module-import cold-start path (see CLAUDE.md import conventions).
    """
    global _title_fragment_classifier
    if _title_fragment_classifier is None:
        from src.anubis.utils.classes.TitleFragmentClassificationClass import (
            TitleFragmentClassificationClass,
        )

        _title_fragment_classifier = TitleFragmentClassificationClass()
    return _title_fragment_classifier
