...
- Choose 'respond' only when the observation carries intent the avatar is meant to answer. At least one of the following must hold, and none of them is satisfied by the conversation partner simply being in frame:
  ...
  (d) The conversation partner is performing for the avatar: making a face, sticking out a tongue, showing nostrils, sucking on an arm or a hand, holding a silly pose, waving, dancing, or repeating a gag at the camera. That is a bid. Choose 'respond', set observation_kind to 'playful_performance', and set salience at or above 0.80. A person sitting still at a desk is not this case. Each new beat of the same gag is a new bid — do not choose 'ignore' because an earlier observation already drew a reply for a similar face.
...
- Do not repeat, in different words, something an earlier observation in this conversation already prompted the avatar to say. Read EARLIER_OBSERVATIONS before deciding, and choose 'ignore' when the avatar would only be saying the same thing again. A playful_performance is the exception: another face, another tongue, another gag is a new bid even when the last reply was already a reaction.
